...
CAMINHO_PASTA_EXCEL = r"C:\Users\ander\Documents\Projetos\PainelPrioridades-main\dados"
...
class PainelMtec(QMainWindow):
    ...
    def inicializar_banco_de_dados(self):
        """Cria o arquivo de banco de dados e a tabela 'concluidos' se não existirem."""
        try:
    ...
    def setup_ui(self):
        ...
        self.body_layout.setContentsMargins(15, 15, 15, 15); self.body_layout.setSpacing(20)
    ...
    def setup_ui_columns(self):
        ...
        self.prioridades_layout.setSpacing(15)
        ...
        self.em_montagem_layout.setContentsMargins(0, 20, 0, 0); self.em_montagem_layout.setSpacing(0)
    ...
    def on_sync_history_clicked(self):
        ...
        try:
            ...
            # --- CORREÇÃO: Lógica de data mais robusta ---
            # Cria uma coluna de data segura, convertendo erros em valores nulos (NaT)
    ...
    def desenhar_dashboard(self, metricas, dados_grafico, frase_do_dia):
        ...
        titulo_grafico = QLabel(f"Desempenho Semanal (Meta: {META_SEMANAL} máquinas)"); titulo_grafico.setFont(titulo_metrica_font)
        ...
        sync_button.setFixedSize(140, 28)
    ...
    def criar_card_widget(self, data, pos_lista):
        ...
        titulo.setFont(QFont("Inter", 12, QFont.Bold))
        titulo.setWordWrap(True)

        status_text = str(data.get(COLUNA_STATUS, 'N/A')).upper()
        status = QLabel(status_text)
        status.setFont(QFont("Inter", 10, QFont.Bold))
        
        current_status = str(data[COLUNA_STATUS]).strip().lower()
        if current_status == STATUS_URGENTE.lower():
            status.setObjectName("CardStatus_Urgente")
        elif current_status == STATUS_AGUARDANDO.lower():
            status.setObjectName("CardStatus_Aguardando")
        elif current_status == STATUS_EM_MONTAGEM.lower():
            status.setObjectName("CardStatus_EmMontagem")
        
        equipamento_texto = str(data.get(COLUNA_EQUIPAMENTO, ''))
        equipamento = QLabel(equipamento_texto)
        equipamento.setWordWrap(True)
        equipamento.setFont(QFont("Inter", 10)) 
        equipamento.setStyleSheet("color: #E0E0E0;")

        servico = QLabel(str(data.get(COLUNA_SERVICO, 'N/A')))
        servico.setWordWrap(True)
        servico.setFont(QFont("Inter", 9, italic=True))
        servico.setStyleSheet("color: #AAAAAA;")

        qtd = QLabel(f"<b>QTD. MÁQUINAS:</b> {data[COLUNA_QTD]}") 
        qtd.setFont(QFont("Inter", 10, QFont.Bold))
            ...
            for _, row in df.head(5).iterrows():
                ...
                if "EM MONTAGEM" in titulo_texto:
                    ...
                    if equip_texto:
                        ...
                        equip_label.setFont(QFont("Inter", 9, italic=True)); equip_label.setStyleSheet("color: #AAAAAA; padding-left: 10px;"); layout.addWidget(equip_label)
